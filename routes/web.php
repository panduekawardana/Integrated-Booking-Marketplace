...
Route::get('/', function () {
    return Inertia::render('Welcome', [
        ...
        'travelPackages' => TravelPackage::with('media')->latest()->take(6)->get(),
        'motorRentals' => MotorRental::with('media')->latest()->take(6)->get(),
        'tourGuides' => TourGuide::with('media')->latest()->take(6)->get(),
        'homestays' => Homestay::with('media')->latest()->take(6)->get(),
    ]);
})->name('home');
...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::get('/bookings/{booking}/payment', [PaymentController::class, 'process'])->name('customer.payment.process');
    Route::get('/payments/finish', [PaymentController::class, 'finish'])->name('customer.payment.finish');
    Route::get('/payments/unfinish', [PaymentController::class, 'unfinish'])->name('customer.payment.unfinish');
    Route::get('/payments/error', [PaymentController::class, 'error'])->name('customer.payment.error');
...
// Public service listing pages - accessible without login
